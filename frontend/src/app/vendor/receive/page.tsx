  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/vendors?user_id=${user.id}`)
      ...
      .then((vendors: VendorProfile[]) => {
        if (vendors.length > 0) setVendor(vendors[0]);
      });
  }, [router]);
      ...
      <Card style={{ position: 'relative', overflow: 'hidden' }}>
        ...
        <CardContent style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2.5rem', zIndex: 1, position: 'relative' }}>
          {vendor ? (
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: '1.1rem' }}>
              Register as a vendor first to receive payments.
            </p>
          )}
          ...
          <div style={{
            ...
            transition: 'box-shadow 1s ease-in-out', padding: '1.5rem', position: 'relative'
          }}>
          ...
          {vendor && (
            <div style={{ width: '100%' }}>
              ...
              <div style={{
                ...
                letterSpacing: '1px', color: 'var(--accent-primary)',
                display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem'
              }}>
                {vendor.business_name} (ID: {vendor.id})
              </div>
            </div>
          )}
        </CardContent>
